...
export default function App() {
  ...
  const openFieldPanel=(tab:typeof fieldPanelTab)=>{setFieldPanelTab(tab);setRightOpen(true)};const left=<ToolRail onOpenFieldPanel={openFieldPanel}/>;const right=mode==='sketch'?<PropertiesPanel/>:mode==='field-note'?<FieldNotePanel tab={fieldPanelTab} onTabChange={setFieldPanelTab}/>:measureWorkspace==='lot'?<LotPanel/>:measureWorkspace==='road'?<RoadPanel/>:measureWorkspace==='offset'?<OffsetPanel/>:measureWorkspace==='fence'?<SiteFeaturePanel kind="fence"/>:measureWorkspace==='surface'?<SiteFeaturePanel kind="surface"/>:<MeasurementPanel/>;
