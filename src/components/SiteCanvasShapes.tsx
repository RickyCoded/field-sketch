...
export function RoadShape({road}:{road:RoadMeasurement}){
  ...
  const selectRoad=()=>{const s=useSketchStore.getState();s.setMode('measure');s.setMeasureWorkspace('road');useSketchStore.setState({selectedRoadId:road.id})};
  return <Group onClick={e=>{e.cancelBubble=true;selectRoad()}} onTap={e=>{e.cancelBubble=true;selectRoad()}}>
...
export function FenceShape({ fence }: { fence: MeasuredFence }) {
  ...
  const selected=useSketchStore(s=>s.selectedFenceId===fence.id);const selectFence=()=>{const s=useSketchStore.getState();s.setMode('measure');s.setMeasureWorkspace('fence');useSketchStore.setState({selectedFenceId:fence.id})};
  return <Group onClick={e=>{e.cancelBubble=true;selectFence()}} onTap={e=>{e.cancelBubble=true;selectFence()}}>
    {fence.lineStyle === 'double' && <Line points={points} closed={fence.closed} stroke="#27322e" strokeWidth={7}/>} 
    <Line points={points} closed={fence.closed} stroke={selected?'#d7781f':fence.lineStyle === 'double' ? '#fff' : '#27322e'} strokeWidth={selected?5:3} dash={dash} lineJoin="round" hitStrokeWidth={18}/>
...
export function SurfaceShape({ surface }: { surface: MeasuredSurface }) {
  ...
  const selected=useSketchStore(s=>s.selectedSurfaceId===surface.id);const selectSurface=()=>{const s=useSketchStore.getState();s.setMode('measure');s.setMeasureWorkspace('surface');useSketchStore.setState({selectedSurfaceId:surface.id})};
  return <Group onClick={e=>{e.cancelBubble=true;selectSurface()}} onTap={e=>{e.cancelBubble=true;selectSurface()}}><Line points={points} closed={closed} stroke={selected?'#d7781f':'#58615d'} strokeWidth={selected?5:surface.geometryMode === 'polyline-width' ? (surface.width ?? 2) * 8 : 2} opacity={surface.geometryMode === 'polyline-width' ? .35 : 1} fill={closed ? '#e8e9e6' : undefined} fillPatternImage={pattern} fillPatternRepeat="repeat" lineJoin="round" hitStrokeWidth={18}/><Text x={surface.labelPosition.x} y={surface.labelPosition.y} text={`${surface.label}${surface.areaState === 'covered' ? ' · COVERED' : ''}`} fontSize={11} fontStyle="bold" fill="#4c5551" draggable onDragEnd={e => update(surface.id, { labelPosition: { x: e.target.x(), y: e.target.y() } })}/></Group>;
}
